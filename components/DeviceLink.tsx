...
function Node({ node, state }: { node: LinkNode; state: LinkState }) {
...
export default function DeviceLink({
  ...
  return (
    <div className="dl" role="img" aria-label={described}>
      <Node node={from} state={state} />
      <div className="dl-wire" data-state={state}>
        <span className="dl-track" />
        <span className="dl-fill" style={{ width: `${Math.min(100, pct)}%` }} />
        {state === "moving" && (
          <>
            <span className="dl-pkt" style={{ animationDelay: "0s" }} />
            <span className="dl-pkt" style={{ animationDelay: "0.45s" }} />
            <span className="dl-pkt" style={{ animationDelay: "0.9s" }} />
          </>
        )}
        {state === "done" && (
